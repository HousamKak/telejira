...
class TelegramJiraBot:
    ...
    def _register_handlers(self) -> None:
        ...
        if not self.application:
            raise RuntimeError("Application must be initialized before registering handlers")
        
        if not all([self.base_handler, self.admin_handlers, self.project_handlers, 
                   self.issue_handlers, self.wizard_handlers]):
